function features_vector = haralick_features(glcm)
    ...
    for count = 1:length(glcm)
        ...
        meanx = sum(sum(meanx_index*glcm{count}));
        meany_index = 1:size(glcm{count},2);
        meany = sum(sum(meany_index*(glcm{count}')));
        ...
        HaralickF2(count) = sum(sum(((meshgridy-meshgridx).^2).*glcm{count}));
        HaralickF5(count) = sum(sum(glcm{count}./(1+((meshgridy-meshgridx).^2))));
        F3 = (meshgridy-meanx).*(meshgridx-meany).*glcm{count};
        ...
        HaralickF6(count) =  sum((pxplusy_index+1).*pxplusy);
        HaralickF7(count) = sum((((pxplusy_index+1)-HaralickF6(count)).^2).*pxplusy);
        ...
        %save speed
        HaralickF10(count) = sum((((pxminusy_index-1)-diffavg).^2).*pxminusy);
